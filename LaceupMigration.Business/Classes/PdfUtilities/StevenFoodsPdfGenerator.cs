...
{
    public class StevenFoodsPdfGenerator : IPdfProvider
    {
        #region General
        ...
        protected virtual void AddCompanyInfo(Document doc, Order order)
        {
            ...
            AddCellCustomToBody(tableLayout, (company.CompanyName != null ? company.CompanyName : string.Empty), HorizontalAlignment.LEFT, Border.NO_BORDER, GetBigFont());
            ...
            AddCellCustomToBody(tableLayout, docNum, HorizontalAlignment.RIGHT, Border.NO_BORDER, GetBigFont());


            var text1 = company.CompanyAddress1;

            if (!string.IsNullOrEmpty(company.CompanyAddress2))
                text1 += "\n" + company.CompanyAddress2;


            var text2 = currentDate;
            ...
            AddCellCustomToBody(tableLayout, text1, HorizontalAlignment.LEFT, Border.NO_BORDER, GetNormalFont());
            AddCellCustomToBody(tableLayout, text2, HorizontalAlignment.RIGHT, Border.NO_BORDER, GetNormalFont());
        ...
        protected virtual void AddCellToCustomBody(Table tableLayout, string cellText, int colSpan, int alignment)
        {
            var paragraph = new Paragraph(cellText);
            paragraph.AddStyle(GetSmall8Font());
        ...
        protected virtual void AddCellToBody(Table tableLayout, string cellText, HorizontalAlignment alignment)
        {
            var paragraph = new Paragraph(cellText);
            paragraph.AddStyle(GetSmall8Font());
            paragraph.SetTextAlignment((TextAlignment)(int)alignment);
        ...
        protected virtual void AddCellCustomToBody(Table tableLayout, string cellText, HorizontalAlignment alignment, Border border, Style font)
        {
            var paragraph = new Paragraph(cellText);
        ...
        {
            float[] headers = { 8, 10, 27, 11, 11, 11, 11, 11 };  //Header Widths
            ...
            if (order.Details != null)
            {
                ...
                foreach (OrderDetail detail in SortDetails.SortedDetails(order.Details))
                {
                    ...
                    var baseUOm = product.UnitOfMeasures.FirstOrDefault(x => x.IsBase);
                    bool cameFromOffer = false;
                    var unitPrice = Product.GetPriceForProduct(product, order, out cameFromOffer, false, false, baseUOm);

                    if (Config.HidePriceInSelfService && (Config.SelfServiceUser || Config.SelfService) || Config.HidePriceInTransaction)
                        AddCellToBody(tableLayout, string.Empty, HorizontalAlignment.RIGHT);
                    else
                        AddCellToBody(tableLayout, unitPrice.ToCustomString(), HorizontalAlignment.RIGHT);
        ...
        {
            float[] headers1 = { 8, 10, 27, 11, 11, 11, 11, 11 };  //Header Widths

            Table tableLayout1 = new Table(UnitValue.CreatePercentArray(headers1)).UseAllAvailableWidth();

            var totalItems = order.CalculateOrderTotalItems();

            var totalPrice = order.Details.Sum(x => x.Price);
            var total = order.OrderTotalCost();

            AddCellToCustomBody(tableLayout1, "Total Pieces", 1, (int)TextAlignment.RIGHT);
            AddCellToCustomBody(tableLayout1, totalItems.ToString(), 4, (int)TextAlignment.RIGHT);
            AddCellToCustomBody(tableLayout1, "", 1, (int)TextAlignment.RIGHT);
            AddCellToCustomBody(tableLayout1, "", 1, (int)TextAlignment.LEFT);
            AddCellToCustomBody(tableLayout1, total.ToCustomString(), 1, (int)TextAlignment.RIGHT);
            ...
            Cell signature = new Cell();
            var text = new Paragraph("Receiver Name");
            ...
            //#0012619
            AddCellToHeader(tableLayout, order.SignatureName ?? string.Empty);
            ...
            tableLayout.AddCell(GetSignatureCell(order, 4));
            ...
            if (Config.HidePriceInSelfService && (Config.SelfServiceUser || Config.SelfService) || Config.HidePriceInTransaction)
                ...
                AddCellToBody(tableLayout, (total - payment).ToCustomString(), HorizontalAlignment.RIGHT);
...
.SetTextAlignment(iText.Layout.Properties.TextAlignment.LEFT);
        ...
        //public void AddOrderHeaderTable(Document doc, BankDeposit order, List<InvoicePayment> payments)
        //{
        //    AddTextLine(doc, "DEPOSIT PAYMENT", GetBigFont(), TextAlignment.CENTER);
        //    AddTextLine(doc, Environment.NewLine, GetBigFont());

        //    // Define header widths
        //    float[] headers = { 40, 12, 12, 12, 12, 12 };
        ...
        //    AddCellToHeader(tableLayout, "CUSTOMER");
        //    AddCellToHeader(tableLayout, "DOC#");
        //    AddCellToHeader(tableLayout, "AMOUNT");
        //    AddCellToHeader(tableLayout, "METHOD");
        //    AddCellToHeader(tableLayout, "PAID");
        //    AddCellToHeader(tableLayout, "BALANCE");

        //    // Group payments by client and then by invoices
        //    var groupedByClient = payments.GroupBy(p => p.Client.ClientName)
        //                                  .OrderBy(g => g.Key);

        //    foreach (var clientGroup in groupedByClient)
        //    {
        //        double totalAmountForClient = 0;
        //        double totalPaidForClient = 0;

        //        foreach (var payment in clientGroup)
        //        {
        //            var invoices = payment.Invoices().OrderBy(i => i.InvoiceNumber);
        //            foreach (var invoice in invoices)
        //            {
        //                double amountPaidForInvoice = payment.Components
        //                                                     .Where(c => c. invoice.InvoiceId)
        //                                                     .Sum(c => c.Amount);
        //                double balanceForInvoice = invoice.Amount - amountPaidForInvoice;

        //                AddCellToBody(tableLayout, clientGroup.Key);
        //                AddCellToBody(tableLayout, invoice.InvoiceNumber);
        //                AddCellToBody(tableLayout, invoice.Amount.ToCustomString());

        //                AddCellToBody(tableLayout, amountPaidForInvoice.ToCustomString());
        //                AddCellToBody(tableLayout, balanceForInvoice.ToCustomString());

        //                totalAmountForClient += invoice.Amount;
        //                totalPaidForClient += amountPaidForInvoice;
        //            }
        //        }

        //        double balanceForClient = totalAmountForClient - totalPaidForClient;

        //        // Add a total row for the client
        //        AddCellToHeader(tableLayout, "TOTAL INVOICES");
        //        AddCellToHeader(tableLayout, totalAmountForClient.ToCustomString());
        //        AddCellToHeader(tableLayout, "TOTAL PAID");
        //        AddCellToHeader(tableLayout, totalPaidForClient.ToCustomString());
        //        AddCellToHeader(tableLayout, "BALANCE");
        //        AddCellToHeader(tableLayout, balanceForClient.ToCustomString());
        //    }
        ...
        //                double amountToSubstract = i.Amount;
        ...
        //                    AddCellToBody(tableLayout1, clientPayments.Client.ClientName);
        //                    AddCellToBody(tableLayout1, i.InvoiceNumber);
        //                    AddCellToBody(tableLayout1, i.Amount.ToCustomString());
        ...
        //                    double b = i.Amount - listOfComponentsUsed.Sum(x => x.AmountRemaining);
        ...
        //        var totalBalanceInvoices = clientPayments.Payments.Sum(x => x.Invoices().Sum(y => y.Amount));
        //        var totalPaid = clientPayments.Payments.Sum(x => x.TotalPaid);
        //        var balance = totalBalanceInvoices - totalPaid;


        //        AddCellToHeader(tableLayout1, "TOTAL INVOICES");
        //        AddCellToHeader(tableLayout1, totalBalanceInvoices.ToCustomString());
        //        AddCellToHeader(tableLayout1, "TOTAL PAID");
        //        AddCellToHeader(tableLayout1, totalPaid.ToCustomString());
        //        AddCellToHeader(tableLayout1, "BALANCE");
        //        AddCellToHeader(tableLayout1, balance.ToCustomString());
        //    }
        ...
        protected virtual void AddClientStatementHeaderToPDF(Document doc, Client client)
        {
            ...
            // Las siguientes cuatro partes (ciudad, estado, código postal y un espacio adicional) se unen.
            // Asegúrate de que haya suficientes partes.
            string cityStateZip = addressParts.Length > 3
                ? string.Join(" ", addressParts[1], addressParts[2], addressParts[3], addressParts[4])
