...
{
    public class GlenWillsPdfGenerator : IPdfProvider
    {
        #region General
        ...
        {
            float[] headers = { 30, 10, 10, 15, 10, 10, 15 };  //Header Widths

            //Create PDF Table
            Table tableLayout = new Table(UnitValue.CreatePercentArray(headers)).UseAllAvailableWidth();

            AddCellToHeader(tableLayout, "Item");
            AddCellToHeader(tableLayout, "Qty");
            AddCellToHeader(tableLayout, "UoM");
            AddCellToHeader(tableLayout, "Standard Price");
            ...
            if (order.Details != null)
            {
                ...
                foreach (OrderDetail detail in SortDetails.SortedDetails(order.Details))
                {
                    ...
                    AddCellToBody(tableLayout, listPrice.ToCustomString(), HorizontalAlignment.RIGHT);

                    double discount = 0;
                    if (detail.Price < listPrice && !detail.IsCredit && !detail.IsFreeItem && showDiscount)
                        discount = ((detail.Price - listPrice));
        ...
        protected virtual void AddFooterTable(Document doc, Order order)
        {
            ...
            foreach (var detail in order.Details)
            {
                if (detail.IsCredit)
                {
                    subtotal += ((detail.Price * detail.Qty) * -1);
                    continue;
                }
            ...
            if (Config.HidePriceInSelfService && (Config.SelfServiceUser || Config.SelfService) || Config.HidePriceInTransaction)
                ...
                AddCellToBody(tableLayout, subtotal.ToCustomString(), HorizontalAlignment.RIGHT);
            ...
            foreach (var d in order.Details)
            {
                if (!d.IsCredit && !d.IsFreeItem)
                {
                    ...
                    if (d.Price < d.Product.PriceLevel0 && showDiscount)
                        discount += ((d.Price - d.Product.PriceLevel0) * d.Qty);
                }
            }
