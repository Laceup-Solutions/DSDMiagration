...
{
    public class DistLaEsperanzaPrinter : ZebraFourInchesPrinter1
    {
        const string LaEsperanzaFooterLine = "LaEsperanzaFooterLine";
        const string LaEsperanzaFooterLine1 = "LaEsperanzaFooterLine1";

        protected override void FillDictionary()
        {
            base.FillDictionary();

            linesTemplates.Add(LaEsperanzaFooterLine, "^FO40,{0}^ADN,18,10^FDReceived By: ________ PymtMet: _______  Collected By: _______^FS");
            linesTemplates.Add(LaEsperanzaFooterLine1, "^FO40,{0}^ADN,18,10^FDPending: _______      Date: _________^FS");
        }
        ...
        protected override IEnumerable<string> GetTotalsRowsInOneDoc(ref int startY, Client client, Dictionary<string, OrderLine> sales, Dictionary<string, OrderLine> credit, Dictionary<string, OrderLine> returns, InvoicePayment payment, Order order)
        {
            ...
            list.Add(string.Format(CultureInfo.InvariantCulture, linesTemplates[LaEsperanzaFooterLine], startY));
            startY += font18Separation;
            startY += font18Separation;

            list.Add(string.Format(CultureInfo.InvariantCulture, linesTemplates[LaEsperanzaFooterLine1], startY));
