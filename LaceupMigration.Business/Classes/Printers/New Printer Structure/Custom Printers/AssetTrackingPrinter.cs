...
{
    class AssetTrackingPrinter : ZebraFourInchesPrinter1
    {
        protected const string ContainerHeader = "ContainerHeader";
        protected const string ContainerHeader1 = "ContainerHeader1";
        protected const string ContainerProductHeader1 = "ContainerProductHeader1";
        protected const string ContainerProductHeader2 = "ContainerProductHeader2";
        protected const string ContainerProductHeader3 = "ContainerProductHeader3";
        protected const string ContainerProductDetail = "ContainerProductDetail";
        ...
        protected override void FillDictionary()
        {
            ...
            //outter table
            linesTemplates.Add(ContainerHeader, "^FO40,{0}^CF0,45^FB620,1,0,L^FDCASES^FS");
            linesTemplates.Add(ContainerProductHeader1, "^FO40,{0}^CF0,33^FB620,1,0,L^FDPRODUCT^FS^FO400,{0}^CF0,33^FB620,1,0,L^FDUNITS IN^FS ^FO600,{0}^CF0,33^FB620,1,0,L^FDAMOUNT^FS");
            linesTemplates.Add(ContainerProductHeader2, "^FO40,{0}^CF0,33^FB620,1,0,L^FDPRODUCT^FS^FO400,{0}^CF0,33^FB620,1,0,L^FDUNITS OUT^FS ^FO600,{0}^CF0,33^FB620,1,0,L^FDAMOUNT^FS");
            linesTemplates.Add(ContainerProductHeader3, "^FO400,{0}^AFN,18,18^FD{1}^FS^FO600,{0}^AFN,18,18^FD{2}^FS");

            linesTemplates.Add(ContainerProductDetail, "^FO40,{0}^AFN,18,18^FD{1}^FS^FO400,{0}^AFN,18,18^FD{2}^FS^FO600,{0}^AFN,18,18^FD{3}^FS");
            linesTemplates.Add(ContainerHeader1, "^FO40,{0}^CF0,38^FB620,1,0,L^FD****CASES OUTSTANDING: {1}^FS");
        ...
        protected override IEnumerable<string> GetTotalsRowsInOneDoc(ref int startY, Client client, Dictionary<string, OrderLine> sales, Dictionary<string, OrderLine> credit, Dictionary<string, OrderLine> returns, InvoicePayment payment, Order order)
        {
            ...
            var current  = AssetTracking.List.FirstOrDefault(x => x.Client.ClientId == client.ClientId);


            double totalPrice = 0;
            if (current != null)
            {
                var price = Product.GetPriceForProduct(current.Product, current.Client, false, false);

                double cratesInPrice = (current.Ins * price);
                double cratesOutPrice = (current.Outs * price);
                totalPrice = cratesInPrice - cratesOutPrice;

                startY -= 60;
                //here do the container stuff
                list.Add(string.Format(CultureInfo.InvariantCulture, linesTemplates[ContainerHeader], startY));
                startY += 60;
                list.Add(string.Format(CultureInfo.InvariantCulture, linesTemplates[ContainerProductHeader1], startY));
                startY += 40;
                list.Add(string.Format(CultureInfo.InvariantCulture, linesTemplates[ContainerProductDetail], startY, current.Product.Name, current.Ins, cratesInPrice.ToCustomString()));
                startY += 40;
                list.Add(string.Format(CultureInfo.InvariantCulture, linesTemplates[ContainerProductHeader2], startY));
                startY += 40;
                list.Add(string.Format(CultureInfo.InvariantCulture, linesTemplates[ContainerProductDetail], startY, current.Product.Name, current.Outs, cratesOutPrice.ToCustomString()));
                startY += 40;
                var s = string.Empty;
                ...
                list.Add(string.Format(CultureInfo.InvariantCulture, linesTemplates[ContainerProductHeader3], startY, (current.Ins - current.Outs), totalPrice.ToCustomString()));
                startY += 40;
                startY += 40;

                list.Add(string.Format(CultureInfo.InvariantCulture, linesTemplates[ContainerHeader1], startY, current.LastOH + (current.Ins - current.Outs)));
                startY += 40;
                startY += 40;
            }
            ...
            foreach (var key in sales.Keys)
            {
                ...
                {
                    if (od.Product.ProductType == ProductType.Asset)
                        continue;
            ...
            if (!Config.HideTotalOrder && printTotal)
            {
                ...
                if(current != null)
                {
                    s1 = ToString(Math.Round(totalPrice, Config.Round));
                    s1 = new string(' ', 14 - s1.Length) + s1;
                    list.Add(string.Format(CultureInfo.InvariantCulture, linesTemplates[OrderTotalContainers], startY, s1));
                    startY += font36Separation;
                }
        ...
        protected override IEnumerable<string> GetSectionRowsInOneDoc(ref int startIndex, IList<OrderLine> lines, string sectionName, int factor, Order order, bool preOrder)
        {
            ...
            {
                if (detail.Product.ProductType == ProductType.Asset)
                    continue;
