...
{
    public class GS1_128Decoder : BarcodeDecoder
    {
        public GS1_128Decoder(string s) : base(s)
        {
            var result = EAN128Parser.Parse(s);
            ...
            foreach (var item in result)
            {
                switch (item.Key.AI)
                {
                    ...
                    case "15":
                        try
                        {
                            Expiration = DateTime.ParseExact(item.Value, "yyMMdd", System.Globalization.CultureInfo.InvariantCulture);
                        }
                        catch (Exception ex)
                        {
                            Logger.CreateLog(ex);
                    ...
                    case "11":
                        try
                        {
                            var productionDate = DateTime.ParseExact(item.Value, "yyMMdd", System.Globalization.CultureInfo.InvariantCulture);
                            Expiration = productionDate.AddDays(365);
                        }
                        catch (Exception ex)
                        {
                            Logger.CreateLog(ex);
                    ...
                    case "10":
                        int index = -1;
                        string v = item.Value;
                        while ((index = v.IndexOf((char)29)) != -1)
                        {
                            var subString = v.Substring(index + 1);
                            v = v.Remove(index) + subString;
                        }
                        Lot = v;
                        break;
                    case "21":
                        ...
                        while ((index1 = v1.IndexOf((char)29)) != -1)
                        {
                            var subString = v1.Substring(index1 + 1);
                            v1 = v1.Remove(index1) + subString;
                        }
                        Lot = v1;
                        break;
                    case "30":
                        float qty = 0;
                        float.TryParse(item.Value, out qty);
                        Qty = qty;
                        break;
                    case "320d":
                        int decimals = 0;
                        int.TryParse(item.Key.Dvalue, out decimals);
                        double weight = 0;
                        double.TryParse(item.Value, out weight);
                        Weight = (float)(weight / Math.Pow(10, decimals));
                        Weight = (float)Math.Round(Weight, 2);
                        break;
                    case "310d":
                        int decimals1 = 0;
                        int.TryParse(item.Key.Dvalue, out decimals1);
                        double weight1 = 0;
                        double.TryParse(item.Value, out weight1);
                        Weight = (float)(weight1 / Math.Pow(10, decimals1));
                        Weight = (float)Math.Round(Weight, 2);
                        break;
                    default:
                        break;
        ...
        public static class EAN128Parser
        {
            ...
            public class AII
            {
                ...
                {
                    return String.Format("{0} [{1}] [{2}]", AI, Description, Dvalue);
                }
            }
            ...
            static EAN128Parser()
            {
                ...
                Add("16", "SellByDate_JJMMDD", 2, DataType.Numeric, 6, false);
            ...
            private static void Add(string AI, string Description, int LengthOfAI, DataType DataDescription, int LengthOfData, bool FNC1)
            {
            ...
            private static AII GetAI(string data, ref int index, bool usePlaceHolder = false)
            {
                ...
                for (int i = minLengthOfAI; i <= maxLengthOfAI; i++)
                {
                    try
                    {
                        ...
                        if (usePlaceHolder)
                        {
                            dValue = ai[ai.Length - 1].ToString();
