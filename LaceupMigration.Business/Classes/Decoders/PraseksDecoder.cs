...
{
    public class PraseksDecoder : BarcodeDecoder
    {
        public PraseksDecoder(string s) : base(s)
        {
            var result = GS1.Parse(s);
            ...
            foreach (var item in result)
            {
                switch (item.Key.AI)
                {
                    case "01":
                        UPC = item.Value;
                        Product = ActivityExtensionMethods.GetProduct(null, UPC, false);
                        break;
                    case "15":
                        try
                        {
        ...
        public static class GS1
        {
            ...
            static GS1()
            {
                ...
                Add("16", "ExpiryDate_JJMMDD", 2, DataType.Numeric, 6, false);
            ...
            private static AII GetAI(string data, ref int index, bool usePlaceHolder = false)
            {
                ...
                if (char.GetNumericValue(data[index]) == -1)
                    index++;
