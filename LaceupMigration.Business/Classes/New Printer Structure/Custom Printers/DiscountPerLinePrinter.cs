...
{
    class DiscountPerLinePrinter : ZebraFourInchesPrinter1
    {
        protected const string NathanWholesaleDetail = "NathanWholesaleDetail";
        ...
        protected override void FillDictionary()
        {
            ...
            linesTemplates[OrderDetailsHeader] = "^FO40,{0}^ADN,18,10^FDPRODUCT^FS" +
                ...
                "^FO580,{0}^ADN,18,10^FDL.PRICE^FS" +
                "^FO680,{0}^ADN,18,10^FDTOTAL^FS";

            linesTemplates.Add(NathanWholesaleDetail, "^FO580,{0}^ADN,18,10^FDDISC.^FS" +
                "^FO680,{0}^ADN,18,10^FDF.PRICE^FS" +
                "^FO580,{1}^ADN,18,10^FD{2}^FS" +
                "^FO680,{1}^ADN,18,10^FD{3}^FS");
            ...
            foreach (var detail in lines)
            {
                ...
                foreach (string pName in productSlices)
                {
                    if (productLineOffset == 0)
                    {
                        ...
                        double listprice = detail.ListPrice;
                        ...
                        {
                            if (detail.OrderDetail.Discount > 0)
                            {
                                if (detail.OrderDetail.DiscountType == DiscountType.Amount)
                                {
                                    discount = detail.OrderDetail.Discount;
                                }
                                else
                                {
                                    discount = price * (detail.OrderDetail.Discount);
                                }
                            }
                        }

                        string discountAsString = ToString(discount);
                        ...
                        list.Add(GetSectionRowsInOneDocFixedLine(OrderDetailsLines, startIndex, pName, qtyAsString, totalAsString, listpriceAsString));
                        startIndex += font18Separation;

                        list.Add(string.Format(CultureInfo.InvariantCulture, linesTemplates[NathanWholesaleDetail], (startIndex + font18Separation), (startIndex + 50), discountAsString, priceAsString));
                    }
