...
{
    public class DonAnecioPrinter : ZebraThreeInchesPrinter1
    {
        string myline = "^FO15,{0}^ADN,18,10^FD{1}^FS" + "^FO275,{0}^ADN,18,10^FD{2}^FS";
        protected const string OrderDetailsLinesUnits = "OrderDetailsLinesUnits";
        ...
        protected override void FillDictionary()
        {
            ...
            linesTemplates[OrderDetailsLinesUnits] = "^FO275,{0}^ADN,18,10^FD{1}^FS";
        }
        ...
        protected override IEnumerable<string> GetSectionRowsInOneDoc(ref int startIndex, IList<OrderLine> lines, string sectionName, int factor, Order order, bool preOrder)
        {
            ...
            foreach (var detail in lines)
            {
                ...
                if (Config.ConcatUPCToName && !string.IsNullOrEmpty(p.Upc))
                    name = p.Name + " " + p.Upc;
                var units = detail.Qty * detail.OrderDetail.UnitOfMeasure.Conversion;
                var uString = units.ToString() + " Units";
               
                var productSlices = GetOrderDetailsRowsSplitProductName(name);

                /*if (productSlices.Count > 1)
                {
                    if (detail.OrderDetail.UnitOfMeasure.Name == "box")
                        productSlices[1] = productSlices[1] + "         " + units.ToString() + " Units" ;
                }*/
                ...
                foreach (string pName in productSlices)
                {
                    if (productLineOffset == 0)
                    {
                        ...
                        //if (detail.OrderDetail.UnitOfMeasure.Name == "box")
                            //qtyAsString += " " + units.ToString(CultureInfo.CurrentCulture) + " " + "Units";
                    ...
                    else if (!Config.PrintTruncateNames)
                    {
                        ...
                        {
                            if (detail.OrderDetail.UnitOfMeasure.Name == "box")
                                list.Add(string.Format(CultureInfo.InvariantCulture, myline, startIndex, pName, uString));
                        }
                        else
                ...
                if (productSlices.Count == 1)
                {
                    // print second line with package IF NEEDED
                    if (detail.OrderDetail.UnitOfMeasure.Name == "box")
                        list.Add(string.Format(CultureInfo.InvariantCulture, linesTemplates[OrderDetailsLinesUnits], startIndex, uString)); //OrderDetailsLines2
                    startIndex += font18Separation;
                }

                //var units = detail.Qty * detail.OrderDetail.UnitOfMeasure.Conversion;
                //list.Add(string.Format(CultureInfo.InvariantCulture, linesTemplates[OrderDetailsLinesUnits], startIndex, units.ToString(CultureInfo.CurrentCulture)));
                startIndex += font18Separation;
