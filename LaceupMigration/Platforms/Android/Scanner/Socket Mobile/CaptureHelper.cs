    ...
    namespace Capture
    {
        ...
        public partial class CaptureHelper
        {
            ...
            public Task<AsyncResult> SetDataConfirmationActionAsync(int beep, int led, int rumble)
            {
                ...
                property.Ulong = ((rumble << 16) | (beep << 8) | led);
                Task<PropertyResult> result = _capture.SetPropertyAsync(property);
            ...
            public Task<DataConfirmationActionResult> GetDataConfirmationActionAsync()
            {
                ...
                Task<DataConfirmationActionResult> result =
                    propertyResult.ContinueWith<DataConfirmationActionResult>(res => {
                        int beep = 0;
                        int led = 0;
                        int rumble = 0;
                        ...
                        if (res.Result.IsSuccessful())
                        {
                            beep = (int)((res.Result.Property.Ulong >> 8) & 0xFF);
                            led = (int)(res.Result.Property.Ulong & 0xFF);
                            rumble = (int)((res.Result.Property.Ulong >> 16) & 0xFF);
                    ...
                    case ICaptureEvent.Id.kPower:
                        {
                            ...
                            {
                                CaptureHelperDevice deviceFound = _devices.Find(device => device.GetDeviceInfo().Guid == e.captureDevice.Guid);
                                PowerState state = (PowerState)(e.captureEvent.DataLong & 0xFF);
                    ...
                    case ICaptureEvent.Id.kBatteryLevel:
                        {
                            ...
                            {
                                CaptureHelperDevice deviceFound = _devices.Find(device => device.GetDeviceInfo().Guid == e.captureDevice.Guid);
                                int min = (int)(e.captureEvent.DataLong & 0xFF);
                                int max = (int)((e.captureEvent.DataLong >> 8) & 0xFF);
                                int current = (int)((e.captureEvent.DataLong >> 16) & 0xFF);
                    ...
                    case ICaptureEvent.Id.kButtons:
                        {
                            ...
                            {
                                CaptureHelperDevice deviceFound = _devices.Find(device => device.GetDeviceInfo().Guid == e.captureDevice.Guid);
                                bool leftButton = (e.captureEvent.DataByte & 0x01) != 0;
                                bool middleButton = (e.captureEvent.DataByte & 0x02) != 0;
                                bool rightButton = (e.captureEvent.DataByte & 0x04) != 0;
                                bool powerButton = (e.captureEvent.DataByte & 0x08) != 0;
        ...
        public partial class CaptureHelperDevice
        {
            ...
            public string DeviceTypeName
            {
                get
                {
                    ...
                    // Use device name directly from DeviceInfo if available, otherwise use type ID
                    if (!string.IsNullOrEmpty(CaptureDevice.DeviceInfo.Name))
                    {
                        deviceName = CaptureDevice.DeviceInfo.Name;
                    }
                    else
                    {
                        // Fallback to device type ID mapping
                        var deviceType = (int)CaptureDevice.DeviceInfo.Type;
                        deviceName = deviceType switch
                        {
                            0 => "Unknown device",
                            7 => "CHS 7",
                            17 => "CHS 7X",
            ...
            public Task<PowerStateResult> GetPowerStateAsync()
            {
                ...
                property.Type = ICaptureProperty.Types.kNone;
                Task<PropertyResult> resultGetProperty = CaptureDevice.GetPropertyAsync(property);
                Task<PowerStateResult> result = resultGetProperty.ContinueWith<PowerStateResult>(res =>
                {
                    string powerState = "Unknown";
                    CaptureHelper.PowerState state = (CaptureHelper.PowerState)(res.Result.Property.Ulong & 0xFF);
            ...
            public Task<BatteryLevelResult> GetBatteryLevelAsync()
            {
                ...
                property.Type = ICaptureProperty.Types.kNone;
                Task<PropertyResult> resultGetProperty = CaptureDevice.GetPropertyAsync(property);
                Task<BatteryLevelResult> result = resultGetProperty.ContinueWith<BatteryLevelResult>(res =>
                {
                    int minLevel = 0;
                    int maxLevel = 0;
                    int curLevel = 0;
                    ...
                    if (res.Result.IsSuccessful())
                    {
                        minLevel = (int)(res.Result.Property.Ulong & 0xFF);
                        maxLevel = (int)((res.Result.Property.Ulong >> 8) & 0xFF);
                        curLevel = (int)((res.Result.Property.Ulong >> 16) & 0xFF);
            ...
            public Task<NotificationsResult> GetNotificationsAsync()
            {
                ...
                property.Type = ICaptureProperty.Types.kNone;
                Task<PropertyResult> resultGetProperty = CaptureDevice.GetPropertyAsync(property);
                Task<NotificationsResult> result = resultGetProperty.ContinueWith<NotificationsResult>(res =>
                {
                    ...
                    if (res.Result.IsSuccessful())
                    {
                        ...
                        // Scan button notifications (using estimated values)
                        const uint kScanButtonPress = 0x10;
                        const uint kScanButtonRelease = 0x20;
                        
                        if ((res.Result.Property.Ulong & kScanButtonPress) == kScanButtonPress)
                        {
                            scanButtonPress = true;
                        }
                        if ((res.Result.Property.Ulong & kScanButtonRelease) == kScanButtonRelease)
                        {
            ...
            public Task<CaptureHelper.AsyncResult> SetNotificationsAsync(Notifications notifications)
            {
                ...
                {
                    property.Ulong += (int)kScanButtonPress;
                }
                if (notifications.ScanButtonRelease)
                {
                    property.Ulong += (int)kScanButtonRelease;
                }
                Task<PropertyResult> resultSetProperty = CaptureDevice.SetPropertyAsync(property);
                Task<CaptureHelper.AsyncResult> result = resultSetProperty.ContinueWith(res =>
                {
            ...
            public Task<StandConfigurationResult> GetStandConfigurationAsync()
            {
                ...
                Task<StandConfigurationResult> result = resultGetProperty.ContinueWith(res =>
                {
                    StandConfigurationResult standConfigurationResult =
                        new StandConfigurationResult() { Result = res.Result.Result, StandConfiguration = (int)res.Result.Property.Ulong };
                    return standConfigurationResult;
                });
                return result;
            }
            ...
            public class SymbologyResult : CaptureHelper.AsyncResult
            {
                ...
                /// </summary>
                public const int LastSymbologyID = 255; // Updated for new API
                /// <summary>
            ...
            {
                ICaptureProperty property = SktClassFactory.createCaptureProperty();
                // Note: kSymbologyDevice may not be available in new API version
                // Using a fallback property ID for symbology configuration
                property.ID = 0x1010; // Estimated symbology property ID
                property.Type = ICaptureProperty.Types.kByte;
                property.Byte = (byte)SymbologyId;
                Task<PropertyResult> resultGetProperty = CaptureDevice.GetPropertyAsync(property);
                Task<SymbologyResult> result = resultGetProperty.ContinueWith<SymbologyResult>(res =>
                {
                    ...
                    if (res.Result.IsSuccessful())
                    {
                        ...
                        // Simplified status check based on byte value
                        switch (res.Result.Property.Byte)
                        {
                            case 1:
            ...
            public Task<CaptureHelper.AsyncResult> SetSymbologyAsync(int SymbologyId, bool enable)
            {
                ...
                property.ID = 0x1010; // Estimated symbology property ID
                property.Type = ICaptureProperty.Types.kByte;
                property.Byte = enable ? (byte)1 : (byte)0;
                Task<PropertyResult> resultSetProperty = CaptureDevice.SetPropertyAsync(property);
                Task<CaptureHelper.AsyncResult> result = resultSetProperty.ContinueWith(res =>
                {
