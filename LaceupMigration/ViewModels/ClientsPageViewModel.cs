...
namespace LaceupMigration.ViewModels
{
    public partial class ClientsPageViewModel : ObservableObject
    {
        ...
        [ObservableProperty] private bool _showFlatList = true;

        [ObservableProperty] private bool _showGroupedList;

        [ObservableProperty] private bool _isSearchVisible;

        [ObservableProperty] private bool _showSearchButton;

        [ObservableProperty] private bool _searchButtonEnabled;

        [ObservableProperty] private bool _showViewButton;

        [ObservableProperty] private bool _showSelectDayButton;

        [ObservableProperty] private bool _showEditButton;

        [ObservableProperty] private bool _showMapRouteButton;

        [ObservableProperty] private bool _showButtonsLayout;

        [ObservableProperty] private bool _isEditing;

        [ObservableProperty] private string _viewButtonText = "View Route";

        [ObservableProperty] private string _selectDayButtonText = "Other Day";

        [ObservableProperty] private string _editButtonText = "Edit";
            ...
            AdvancedOptionsService advancedOptionsService)
        {
            ...
            MessagingCenter.Subscribe<ConfigurationPageViewModel>(this, "DataCleared",
                _ => { RouteDate = DateTime.Today; });
        }
        ...
        public async Task<bool> RunAppChecks()
        {
            try
            {
        ...
        private async Task RunPendingLoadCheckAfterRenderAsync()
        {
            ...
            if (Config.SyncLoadOnDemand && !Config.NewSyncLoadOnDemand && !Config.OnlyPresale)
            {
                ...
                try
                {
                    await Task.Run(() =>
                    {
                        ...
                        {
                            DataProvider.GetPendingLoadOrders(DateTime.Now, Config.ShowAllAvailableLoads);
                            var pendingOrders = Order.Orders.Where(x =>
                                    (x.OrderType == OrderType.Load || x.IsDelivery) && x.PendingLoad)
                                .ToList();
            ...
            if (!Config.AutoAcceptLoad && Config.PendingLoadToAccept)
            {
                ...
                if (!string.IsNullOrEmpty(loadOrdersResponseMessage))
                    await _dialogService.ShowAlertAsync(loadOrdersResponseMessage, "Alert", "OK");
                else if (hasDeliveries) await Shell.Current.GoToAsync($"acceptload?loadDate={DateTime.Now.Ticks}");
            }
        }
        ...
        {
            if (item == null || item.Client == null) return;

            _appService.RecordEvent("ClientsPage.CustomerSelected");
            
            if(!await RunAppChecks())
                return;

            if (Config.CannotOrderWithUnpaidInvoices && item.HasOverdueInvoices)
            {
                await _dialogService.ShowAlertAsync(
                    "Customers highlighted in red are 90 days late on their payments. Please contact the customer prior to visiting them.",
                    "Alert");
            }
        ...
        private async Task SelectDateAsync()
        {
            ...
            if (initialDate == DateTime.MinValue) initialDate = DateTime.Today;
        ...
        partial void OnIsEditingChanged(bool value)
        {
            ...
            if (Clients.Count == 0 && ClientGroups.Count == 0) return;
        ...
        private async Task RefreshAsync(bool updateList, bool displayMessage)
        {
            ...
            try
            {
                ...
                {
                    await _dialogService.ShowAlertAsync(
                        "Device is running in Demo Mode. Contact Laceup for production access.", "Warning");
        ...
        private void BuildSource()
        {
            ...
            if (Config.AuthorizationFailed) return;

            if (_displayMode == DisplayMode.All)
            {
                foreach (var client in Client.SortedClients())
                {
                    if (client == null || client.SalesmanClient) continue;
            ...
            else
            {
                ...
                else
                {
                    ...
                    var filteredRoutes = RouteEx.Routes.Where(x => x.Date.Date.Subtract(RouteDate).Days <= 0)
                        .OrderBy(x => x.LocallySavedStop)
                        .ToList();

                    foreach (var route in filteredRoutes)
                    {
                        if (route.Date.Date.Subtract(RouteDate).Days < 0 && route.Closed) continue;
                        if (route.Date.Date > RouteDate.AddDays(1)) continue;
                        if (!route.FromDelivery && route.Date.Date != RouteDate) continue;
        ...
        private void ApplyFilter()
        {
            ...
            var filteredEntries = _allEntries.Where(entry =>
                {
                    if (entry.Client == null) return false;
        ...
        {
            var query = SearchQuery?.Trim() ?? string.Empty;
            var shouldGroup = !string.IsNullOrEmpty(Config.GroupClientsByCat) && _displayMode == DisplayMode.All &&
            ...
            if (shouldGroup)
            {
                ...
                foreach (var group in dictionary)
                {
                    ...
                    if (items.Count > 0)
                    {
                        var groupVm = new ClientGroupViewModel(group.Key,
                            new ObservableCollection<ClientListItemViewModel>(items));
                        if (query.Length > 0) groupVm.IsExpanded = true;
        ...
        private void RestrictToOpenClients()
        {
            ...
            var openClients = Order.Orders.Where(o => !o.AsPresale && !o.Finished)
                .Select(o => o.Client)
                .Where(c => c != null)
        ...
        private void UpdateButtons()
        {
            ...
            ShowButtonsLayout = ShowMapRouteButton || ShowViewButton || ShowSelectDayButton || ShowEditButton ||
                                ShowSearchButton;
    ...
    public partial class ClientListItemViewModel : ObservableObject
    {
        ...
        public bool ShowStop =>
            _isRouteView && _entry.Type != ClientEntryType.Regular && !string.IsNullOrEmpty(StopText);
        ...
        public Color NameColor
        {
            get
            {
                if (IsEditing) return Color.FromArgb("#1976D2");
                return _nameColor;
            }
        }
        ...
        public Color StopBadgeBackground =>
            _entry.Type switch
            {
        ...
        public ClientListItemViewModel(ClientEntry entry, bool isRouteView)
        {
            ...
            if (Config.TimeSheetCustomization && Session.sessionDetails != null && Client != null)
            {
                ShowVisitBadge = Session.sessionDetails.Any(x =>
                    x.clientId == Client.ClientId && string.IsNullOrEmpty(x.orderUniqueId) &&
                    x.detailType == SessionDetails.SessionDetailType.CustomerVisit && x.endTime == DateTime.MinValue);
            }
    ...
    public sealed class ClientEntry
    {
        ...
        public static ClientEntry CreateRegular(Client client) => new(client, null, ClientEntryType.Regular, 0, 0);

        public static ClientEntry CreateQuote(Client client, int stop) =>
            new(client, null, ClientEntryType.Quote, stop, stop);

        public static ClientEntry CreateDelivery(Client client) => new(client, null, ClientEntryType.Delivery, 0, 0);
        ...
        public static ClientEntry CreateFromRoute(Client client, RouteEx route)
        {
            ...
            else if (route.Order != null) type = ClientEntryType.Delivery;
